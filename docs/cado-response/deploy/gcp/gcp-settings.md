...
Once Cloud Build is enabled, a principal for Compute Engine will have been created in **'IAM and Admin > IAM'** called `xxxxxxxxxxxx@developer.gserviceaccount.com`, we also need this principal to have the custom role we created in the previous step **'Customer -> \<role_name\>'**. Do this by editing the principal with the pen icon on the right and selecting the appropriate role.
